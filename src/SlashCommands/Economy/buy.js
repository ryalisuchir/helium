...
module.exports = {
  ...
  run: async (client, interaction, args) => {
    ...
    if (givenID.length < 2) return interaction.reply({
			embeds: [
				new EmbedBuilder()
    ...
    if (!iDB) return interaction.reply({
			embeds: [
				new EmbedBuilder()
    ...
    if (buyPrice === undefined) return interaction.reply({
			embeds: [
				new EmbedBuilder()
    ...
    if (cDB) { 
        ...
        if (item !== undefined) {
          if (item.count === iDB.itemLimit) return interaction.reply({
						embeds: [
							new EmbedBuilder()
							.setDescription(`You cannot purchase more than ${iDB.itemLimit} of this item.`)
							.setColor('303136')
							],
						ephemeral: true
					});
        ...
        if (cDB.wallet < iDB.buyPrice * giveVal || cDB.wallet === undefined) {
          return interaction.reply({
						embeds: [
							new EmbedBuilder()
							.setDescription(`You don't have enough money to buy this, don't try to scam me.`)
							.setColor('303136')
						],
						ephemeral: true
					})
        } else if (item === undefined) {
          if (giveVal > iDB.itemLimit) return interaction.reply({
						embeds: [
							new EmbedBuilder()
          ...
          cDB.inventory.push({ itemName: iDB.itemName, itemID: iDB.itemID, userID: interaction.user.id, count: giveVal })
        ...
        await interaction.reply({
					embeds: [
						new EmbedBuilder()
						.setDescription(`You successfully bought ${Number(giveVal).toLocaleString()} ${iDB.itemName} for $${(buyPrice * giveVal).toLocaleString()}!`)
						.setColor('303136')
					],
					ephemeral: true
      ...
      await interaction.reply({
				embeds: [
					new EmbedBuilder()
					.setColor('303136')
					.setDescription(`You don't have enough money to buy this item, don't try to scam me.`)
				],
				ephemeral: true
			}).catch(err => console.log(err));
    }
