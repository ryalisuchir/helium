...
module.exports = {
  ...
  run: async (client, interaction, args) => {
		if (!interaction.user.id === "823933160785838091") return interaction.reply({
			content: 'Coming soon...'
		});
    ...
    if (cDB) {
      ...
      if (lastBet !== null && cooldown - (Date.now() - lastBet) > 0) {
        ...
        return interaction.reply({
					embeds: [
						new EmbedBuilder()
						.setDescription(`You must wait **${timeObj}** before betting again.`)
						.setColor('303136')
					],
					ephemeral: true
				})
        ...
        if (walletBalance === undefined) return interaction.reply({
					embeds: [
						new EmbedBuilder()
        ...
        if (Number(walletBalance) >= maximumWallet) return interaction.reply({
					embeds: [
						new EmbedBuilder()
						.setDescription('You\'re too loaded to gamble your money... why don\'t you spend some of it rather than hoarding it all?')
					]
				})
        if (parseInt(walletBalance) === 0) return interaction.reply({
					embeds: [
						new EmbedBuilder()
						.setDescription('You can\'t bet nothing...')
						.setColor('303136')
        ...
        bet = Math.ceil(bet);
        if (bet > walletBalance) return interaction.reply({
					embeds: [
						new EmbedBuilder()
