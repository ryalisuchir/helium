...
module.exports = new Event("interactionCreate", async (interaction) => {
	...
	if (interaction.customId.startsWith('accept-event-')) {
		...
		if (!serverProfile || !serverProfile.eventsManager || !serverProfile.eventsPing) return interaction.reply({
			embeds: [
				new EmbedBuilder()
					.setDescription(`This guild has not setup event managers and/or event pings. Please use the following commands:
...
<:slash:980152110127669279> setpings <event> <role>`)
		...
		if (!interaction.member.roles.cache.has(managerID) && !interaction.member.permissions.has([PermissionFlagsBits.Administrator])) {
			return interaction.reply({
				embeds: [
					new EmbedBuilder()
						.setDescription('You do not have the needed permissions to accept events.')
						.setColor('303136')
				],
				ephemeral: true
			})
		}
		if (interaction.user.id === donorID) return interaction.reply({
			embeds: [
				new EmbedBuilder()
		...
		let message = await client.channels.cache.get(channelID).messages.fetch(messageID)
		message.reply({
			content: `<@${donorID}>`,
			embeds: [
				new EmbedBuilder()
					.setDescription(`${interaction.user} has accepted your event.
...
//HEIST
		...
		if (!serverProfile || !serverProfile.heistManager || !serverProfile.heistPing) return interaction.reply({
			embeds: [
				new EmbedBuilder()
					.setDescription(`This guild has not setup heist managers and/or heist pings. Please use the following commands:
...
<:slash:980152110127669279> setpings <heist> <role>`)
		...
		if (!interaction.member.roles.cache.has(managerID) && !interaction.member.permissions.has([PermissionFlagsBits.Administrator])) {
			return interaction.reply({
				embeds: [
					new EmbedBuilder()
						.setDescription('You do not have the needed permissions to accept heists.')
						.setColor('303136')
				],
				ephemeral: true
			})
		...
		message.reply({
			...
			embeds: [
				new EmbedBuilder()
					.setDescription(`${interaction.user} has accepted your heist.
Please send them what you requested to donate.`)
					.setFooter({ text: 'Thank you for donating.' })
					.setColor('303136')
			]
		...
		if (!serverProfile || !serverProfile.giveawayManager || !serverProfile.giveawayPing) return interaction.reply({
			embeds: [
				new EmbedBuilder()
					.setDescription(`This guild has not setup giveaway managers and/or giveaway pings. Please use the following commands:
...
<:slash:980152110127669279> setpings <giveaway> <role>`)
		...
		if (!interaction.member.roles.cache.has(managerID) && !interaction.member.permissions.has([PermissionFlagsBits.Administrator])) {
			return interaction.reply({
				embeds: [
					new EmbedBuilder()
						.setDescription('You do not have the needed permissions to accept giveaways.')
						.setColor('303136')
				],
				ephemeral: true
			})
		...
		if (interaction.user.id === donorID) return interaction.reply({
			embeds: [
				new EmbedBuilder()
				.setDescription('You cannot accept an giveaway for which you donated.')
				.setColor('303136')
			],
			ephemeral: true
		})
		...
		message.reply({
			...
			embeds: [
				new EmbedBuilder()
					.setDescription(`${interaction.user} has accepted your giveaway.
Please send them what you requested to donate.`)
					.setFooter({ text: 'Thank you for donating.' })
					.setColor('303136')
			]
