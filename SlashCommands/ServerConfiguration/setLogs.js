...
module.exports = {
	...
	run: async (client, interaction, args) => {
		...
		if (guildSchema.donationLogs) {
			let previousChannel = interaction.guild.channels.cache.get(guildSchema.donationLogs)

			if (previousChannel) {
				interaction.reply({
					embeds: [
						new EmbedBuilder()
							.setDescription(`There exists a donation logs channel for this guild already, in <#${guildSchema.donationLogs}>.
...
Confirm you would like to switch this channel to ${data.channel}.`)
				...
				interaction.reply({
					embeds: [
						new EmbedBuilder()
							.setDescription(`Previously, there was an invalid channel ID setup for donation logs. It has now been changed to ${data.channel}.`)
							.setColor('303136')
					]
				})
			}
			...
			interaction.reply({
				embeds: [
					new EmbedBuilder()
						.setDescription(`Successfully set the donation logs in this guild to ${data.channel}.`)
						.setColor('303136')
				]
			})
		}
