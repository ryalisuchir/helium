	...
	run: async (client, interaction, args) => {
		...
		if (!dSchema) {
			return interaction.reply({
				embeds: [
					new EmbedBuilder()
					.setDescription('This user does not have any donations in the server.')
					.setColor('303136')
				],
				ephemeral: true
			})
...
\` - \` **Total Donations:** ⏣ ${Math.round(dSchema.donations.event + dSchema.donations.giveaway + dSchema.donations.heist).toLocaleString()}`)
				.setColor('303136')
				]
		})
