...
module.exports = {
  name: "leaderboard",
  description: "Check the leaderboard for donations and managers in a specific guild.",
  ...
  options: [
    ...
    {
      name: "managers",
      description: "A leaderboard for the most active managers in a specific guild.",
      type: ApplicationCommandOptionType.Subcommand,
    },
  ],
  /**
  ...
  run: async (client, interaction, args) => {
    ...
    find = find
      ...
      .sort((a, b) => {
        return (b.donations.event + b.donations.giveaway + b.donations.heist) - (a.donations.event + a.donations.giveaway + a.donations.heist);
      });
    ...
    let mapped = find.map((value, index) => {
      ...
      }: ⏣ **${(value.donations.event + value.donations.giveaway + value.donations.heist).toLocaleString()}**`;
    });
    ...
    let leaderBoardEmbed = new EmbedBuilder()
      ...
      .setDescription(`> \`(#${place})\` - ${interaction.user.tag}\n\n${desc}`)
      .setColor("303136")
    ...
    find = find
      .filter((value) => interaction.guild.members.cache.get(value.userID))
      .sort((a, b) => {
        return (b.timesAccepted.event + b.timesAccepted.giveaway + b.timesAccepted.heist) - (a.timesAccepted.event + a.timesAccepted.giveaway + a.timesAccepted.heist);
      });
    ...
    let mapped = find.map((value, index) => {
      return `\`(#${index + 1})\` ${
        client.users.cache.get(value.userID).tag
      }: **${(value.timesAccepted.event + value.timesAccepted.giveaway + value.timesAccepted.heist).toLocaleString()}** accepted`;
    });
